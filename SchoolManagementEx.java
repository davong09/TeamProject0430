...
public class SchoolManagementEx {
    ...
    private static void registerProfessor(Scanner scanner) { // 교수 등록 메소드
        ...
        repo.getProfessors().add(professor); // repo.getProfessors에 방금 저장된 professor 데이터 추가
    ...
    private static void displayProfessor() { // 교수 조회 메소드
        ...
        for (Professor p : repo.getProfessors()) { // repo.getProfessors 데이터를 자료형이 Professor인 변수 p에 넣고 반복
            System.out.println(p.getId() + " " + p.getJumin() + " " + p.getName() +
                    " " + p.getDepartment() + " " + p.getGrade() + " " + p.getHiredate());
        }
    }
    ...
    private static void updateProfessor(Scanner scanner) { // 교수 정보 수정 메소드
        ...
        for (Professor p : repo.getProfessors()) {
            if (p.getId().equals(id)) { // Professor의 getId와 수정할 교수의 ID에 입력된 id와 같으면 아래를 실행한다.
                ...
                // 입력 받은 name jumin grade hiredate department 값을 각각 p.set__에 저장
    ...
    private static void deleteProfessor(Scanner scanner) { // 교수 삭제 메소드
        ...
        String id = scanner.nextLine(); // 삭제할 교수의 ID를 입력 받아 그 값을 id에 저장
        for (int i = 0; i < repo.getProfessors().size(); i++) { // 0부터 repo.getProfessors의 길이 만큼 반복
            if (repo.getProfessors().get(i).getId().equals(id)) { // 만약 입력된 id가 Professors의 getID와 같으면
                repo.getProfessors().remove(i); // repo.getProfessors의 i번째 값을 삭제한다.
...
// 학과 관련 업무 메소드 끝
    ...
    private static void registerTakes (Scanner scanner){ // 성적 등록 메소드
        System.out.println("학번: ");
        String id = scanner.nextLine(); // 등록할 학생의 학번 입력
        System.out.println("과목코드: ");
        String subject = scanner.nextLine(); // 등록할 과목코드 입력
        System.out.println("학점");
        String score = scanner.nextLine(); // 등록할 학점 입력
        Takes takes = new Takes(id, subject, score); // 입력된 값 takes에 저장
        repo.getTakes().add(takes); // repo.getTakes()배열에 입력된 takes 값 저장
        System.out.println("성적 정보가 성공적으로 등록되었습니다.");
    }
    private static void displayTakes () { // 성적 조회 메소드
        System.out.println("등록된 성적 목록");
        System.out.println("학번 과목코드 학점");
        for (Takes t : repo.getTakes()) { // repo.getTakes 값을 변수 t에 저장하고 반복
            System.out.println(t.getId() + " " + t.getSubject() + " " + t.getScore());
            // 학번 과목코드 학점 출력
        }
    }
    private static void updateTakes (Scanner scanner){ // 성적 수정 메소드
        System.out.println("학번을 입력해주세요.");
        String id = scanner.nextLine(); // 수정할 학생의 학번 id에 저장
        System.out.println("해당 학생의 수정할 과목코드를 입력하세요: ");
        String subject = scanner.nextLine(); // 수정할 학생의 과목코드 subject에 저장
        for (Takes t : repo.getTakes()) {
            if (t.getId().equals(id)) { // 입력된 id와 Takes의 getId 값이 같으면 아래 if문 실행
                if (t.getSubject().equals(subject)) { // 입력된 subject와 Takes의 getSubject 값이 같으면 아래 문장 실행
                    System.out.println("수정할 학점: ");
                    String score = scanner.nextLine(); // 수정할 학점을 입력하여 score에 저장

                    t.setScore(score); // 입력된 score 값을 t.setScore(score)에 저장
                    System.out.println("성적 정보가 업데이트 되었습니다.");
                    return;
                }
            }
        }
        System.out.println("해당 학번 또는 과목코드에 해당하는 성적을 찾을 수 없습니다.");
    }

    private static void deleteTakes(Scanner scanner) { // 성적 삭제
        System.out.println("성적을 삭제할 학생의 학번을 입력해주세요");
        String id = scanner.nextLine();
        for ( int i = 0; i < repo.getTakes().size(); i++) { // i = 0부터 반복
            if (repo.getTakes().get(i).getId().equals(id)) { // repo.getTakes().get(i).getId()의 값과 입력된 id값이 같으면
                repo.getTakes().remove(i); // remove(i) i 번째 값 삭제
                System.out.println("학생의 성적 정보가 삭제되었습니다.");
                return;
            }
        }
